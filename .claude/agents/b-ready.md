...
1. Ensure you're on latest main (worktree-safe — never checks out the `main` branch):
   ```bash
...
**In team mode:** The planner should create the branch. Other agents should check out the same branch before starting work. If spawned with a `WORKTREE_DIR`, all git commands must run in that directory.
...
gh api --method POST "repos/bradyoo12/ai-dev-request/pulls" -f title="<issue_title>" -f head="<branch_name>" -f base="main" -f body="## Summary
...
1. Add `on hold` label (REST):
   ```bash
   gh api --method POST "repos/bradyoo12/ai-dev-request/issues/<issue_number>/labels" -f "labels[]=on hold"
   ```
2. Return to latest main (`git fetch origin && git checkout --detach origin/main`) and delete the branch
