...
#### Step 4b-3: Create Ticket for Build/Deploy Failures Using create-ticket Skill

For each code/config failure, use the **/create-ticket** skill to create a ticket that will be picked up in the next cycle.

1. **Gather failure context:**
   ```bash
   # Get the full error log excerpt
   ERROR_LOG=$(gh run view <run_id> --repo bradyoo12/ai-dev-request --log-failed 2>&1 | head -100)

   # Get the failed run details
   RUN_INFO=$(gh run view <run_id> --repo bradyoo12/ai-dev-request --json displayTitle,headSha,headBranch,conclusion,createdAt,event --jq '{title: .displayTitle, sha: .headSha, branch: .headBranch, event: .event}')

   WORKFLOW_NAME=$(echo "$RUN_INFO" | jq -r '.title')
   HEAD_SHA=$(echo "$RUN_INFO" | jq -r '.sha')
   HEAD_BRANCH=$(echo "$RUN_INFO" | jq -r '.branch')
   ```

2. **Find the related PR (if any):**
   ```bash
   # Check if this commit was part of a recently merged PR
   RELATED_PR_INFO=$(gh api "search/issues?q=repo:bradyoo12/ai-dev-request+is:pr+is:merged+sha:$HEAD_SHA&per_page=1" --jq '.items[0] | {number, title, url}')
   RELATED_PR_NUMBER=$(echo "$RELATED_PR_INFO" | jq -r '.number // empty')
   RELATED_PR_TITLE=$(echo "$RELATED_PR_INFO" | jq -r '.title // empty')
   ```

3. **Create a ticket using the create-ticket skill:**

   Invoke: `Skill(skill: "create-ticket", args: "<ticket-description>")`

   Where `<ticket-description>` is formatted as:
   ```
   [CI/CD] Fix build/deploy failure in {WORKFLOW_NAME}

   GitHub Actions workflow '{WORKFLOW_NAME}' failed on branch '{HEAD_BRANCH}'.

   **Failure Details:**
   - Run ID: <run_id>
   - Run URL: https://github.com/bradyoo12/ai-dev-request/actions/runs/<run_id>
   - Branch: {HEAD_BRANCH}
   - Commit: {HEAD_SHA}
   - Related PR: #{RELATED_PR_NUMBER} - {RELATED_PR_TITLE} (if found, else "None")
   ...
   **Root Cause:**
   [Analyze the error log and describe the root cause - e.g., "Missing entity in DbContext.OnModelCreating", "Type error: missing import", "Build error: wrong namespace", etc.]

   **Fix Required:**
   [Describe what needs to be fixed based on the error - e.g., "Add GeneratedAgent entity to AiDevRequestDbContext.OnModelCreating method", "Import missing namespace in file X", etc.]

   **Testing:**
   1. Fix the identified issue locally
   2. Run relevant build/test commands to verify:
      - Frontend: `cd platform/frontend && npm run build && npm test`
      - Backend: `cd platform/backend/AiDevRequest.API && dotnet build && dotnet test`
   3. Commit and create PR
   4. Verify CI passes on the PR
   5. Merge and verify main branch CI passes
   ```

4. **Add a comment to the related PR/ticket (if found):**

   If `RELATED_PR_NUMBER` exists:
   ```bash
   gh api --method POST "repos/bradyoo12/ai-dev-request/issues/$RELATED_PR_NUMBER/comments" \
     -f body="⚠️ **CI Failure After Merge**

GitHub Actions run #<run_id> failed after merging this PR.

**Workflow:** $WORKFLOW_NAME
**Error:** [Brief 1-line summary of the error]
**Logs:** https://github.com/bradyoo12/ai-dev-request/actions/runs/<run_id>

A fix ticket has been created: #<new_ticket_number>

The build/deploy is currently broken on \`main\` branch."
   ```

5. **Log and proceed:**
   ```bash
   echo "✓ Created ticket #<new_ticket_number> for CI failure run #<run_id>"
   if [ -n "$RELATED_PR_NUMBER" ]; then
     echo "✓ Added comment to related ticket #$RELATED_PR_NUMBER"
   fi
   echo "→ This ticket will be picked up in the next b-start cycle"
   ```

6. **Invalidate the cache** so the new ticket appears in the next cycle:
   ```bash
   bash .claude/scripts/gh-project-cache.sh invalidate
   ```
