...
builder.Services.AddScoped<ICodeQualityReviewService, CodeQualityReviewService>();
...
// This runs in all environments so staging/production schemas stay up to date
{
    ...
    try
    {
        ...
        {
            // MigrateAsync may fail on legacy databases where non-idempotent migrations
            // try to create tables that already exist. Log and continue — table creation
            // below will handle any missing tables.
            logger.LogWarning(migEx, "MigrateAsync failed (may be expected on legacy databases). Continuing with manual table creation.");
        }
        ...
        try
        {
            using var mcpConn = new Npgsql.NpgsqlConnection(dbContext.Database.GetConnectionString());
            await mcpConn.OpenAsync();
            using var mcpCmd = mcpConn.CreateCommand();
            mcpCmd.CommandText = """
                ...
                """;
            await mcpCmd.ExecuteNonQueryAsync();
            logger.LogInformation("Ensured mcp_servers table exists.");
        }
        catch (Exception mcpEx)
        {
            logger.LogWarning(mcpEx, "Failed to ensure mcp_servers table. MCP server endpoint may not work.");
        }
