...
public class CodeReviewServiceTests
{
    ...
    public void Constructor_ThrowsWhenNoApiKey()
    {
        var config = new ConfigurationBuilder()
            ...
            .Build();
        var logger = new Mock<ILogger<CodeReviewService>>();

        var originalKey = Environment.GetEnvironmentVariable("ANTHROPIC_API_KEY");
        try
        {
            Environment.SetEnvironmentVariable("ANTHROPIC_API_KEY", null);
            Assert.Throws<InvalidOperationException>(() => new CodeReviewService(config, logger.Object));
        }
        finally
        {
    ...
    public void Constructor_SucceedsWithApiKey()
    {
        ...
        var service = new CodeReviewService(config, logger.Object);
