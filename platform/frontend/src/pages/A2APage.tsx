  ...
  if (loading) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-gray-400">{t('a2a.loading')}</p>
      </div>
    )
  }
                  ...
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-xs text-gray-500">{agent.agentKey}</div>
                      </div>
                    </div>
                    <span className={`px-2 py-0.5 rounded text-xs font-medium ${
                      agent.isActive ? 'bg-green-900/50 text-green-400' : 'bg-gray-700 text-gray-400'
                    }`}>
                  ...
                  {agent.scopes && (
                    <div className="flex flex-wrap gap-1">
                      {agent.scopes.split(',').map((scope) => (
                        <span key={scope} className="px-1.5 py-0.5 bg-gray-700 text-gray-300 rounded text-[10px]">
                          {scope.trim()}
                        </span>
                      ))}
                    </div>
          ...
          {consents.length === 0 ? (
            ...
            <div className="space-y-3">
              {consents.map((consent) => (
                <div key={consent.id} className="bg-gray-800 rounded-xl p-4 flex items-center justify-between">
                  <div>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-gray-800 rounded-2xl p-6 max-w-md w-full">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setShowRegisterDialog(false)} className="text-gray-400 hover:text-white text-2xl">&times;</button>
            </div>
            <div className="space-y-3">
              <div>
                <label className="block text-sm text-gray-400 mb-1">{t('a2a.agents.keyLabel')}</label>
                <input
                  value={regKey}
              ...
              <div>
                <label className="block text-sm text-gray-400 mb-1">{t('a2a.agents.nameLabel')}</label>
                <input
                  value={regName}
              ...
              <div>
                <label className="block text-sm text-gray-400 mb-1">{t('a2a.agents.descLabel')}</label>
                <textarea
                  value={regDesc}
              ...
              <div>
                <label className="block text-sm text-gray-400 mb-1">{t('a2a.agents.scopesLabel')}</label>
                <input
                  value={regScopes}
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-gray-800 rounded-2xl p-6 max-w-md w-full">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setShowConsentDialog(false)} className="text-gray-400 hover:text-white text-2xl">&times;</button>
            </div>
            <div className="space-y-3">
              <div>
                <label className="block text-sm text-gray-400 mb-1">{t('a2a.consents.fromAgent')}</label>
                <select
                  value={consentFrom}
              ...
              <div>
                <label className="block text-sm text-gray-400 mb-1">{t('a2a.consents.toAgent')}</label>
                <select
                  value={consentTo}
              ...
              <div>
                <label className="block text-sm text-gray-400 mb-1">{t('a2a.consents.scopesLabel')}</label>
                <input
                  value={consentScopes}
