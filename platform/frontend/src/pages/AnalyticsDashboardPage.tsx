      ...
      {dashboard && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-gray-900 rounded-xl p-4">
            <div className="text-sm text-gray-400 mb-1">{t('analytics.activeUsers', 'Active Users')}</div>
            <div className="text-2xl font-bold text-blue-400">{dashboard.activeUsers}</div>
          </div>
          <div className="bg-gray-900 rounded-xl p-4">
            <div className="text-sm text-gray-400 mb-1">{t('analytics.totalRequests', 'Total Requests')}</div>
            <div className="text-2xl font-bold text-green-400">{dashboard.totalRequests}</div>
          </div>
          <div className="bg-gray-900 rounded-xl p-4">
            <div className="text-sm text-gray-400 mb-1">{t('analytics.completionRate', 'Completion Rate')}</div>
            <div className="text-2xl font-bold text-yellow-400">{dashboard.completionRate}%</div>
          </div>
          <div className="bg-gray-900 rounded-xl p-4">
            <div className="text-sm text-gray-400 mb-1">{t('analytics.avgBuildTime', 'Avg Build Time')}</div>
            ...
            {funnel.stages.map((stage) => {
              ...
              return (
                <div key={stage.name} className="flex items-center gap-3">
                  <div className="w-40 text-sm text-gray-300 truncate">{stage.name}</div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {usage.map((item) => {
                  ...
                  return (
                    <tr key={item.eventType} className="border-b border-gray-800">
                      ...
                      <td className="py-2 px-3 text-right text-gray-300">{item.uniqueUsers}</td>
        ...
        {trends.length > 0 ? (
          <div className="space-y-2">
            ...
            <div className="flex items-end gap-1 h-40">
              {trends.map((point) => {
                ...
                return (
                  ...
                  >
                    <div className="text-xs text-gray-400 mb-1">{Math.round(point.value)}</div>
                    <div
            ...
            <div className="flex gap-1">
              {trends.map((point) => (
                <div key={point.date} className="flex-1 text-center text-xs text-gray-500 truncate">
                  {point.date.slice(5)}
                </div>
              ))}
            </div>
