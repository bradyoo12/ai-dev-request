...
export default function ApiDocsPage() {
  ...
  if (loading) {
    return <div className="text-warm-400 text-center py-12">{t('apiDocs.loading')}</div>
  }
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center gap-3">
          <button onClick={() => setViewingSpec(null)} className="text-warm-400 hover:text-white text-sm">
        ...
        {viewingSpec.openApiSpecJson && (
          <div className="bg-warm-800 rounded-lg p-4">
            <div className="flex justify-between items-center mb-3">
              <h4 className="text-sm font-medium text-warm-300">{t('apiDocs.specPreview')}</h4>
              <button
                onClick={() => copySpec(viewingSpec.openApiSpecJson!)}
                className="px-3 py-1 bg-warm-700 hover:bg-warm-600 text-warm-300 rounded text-xs"
              >
                {t('apiDocs.copySpec')}
              </button>
            </div>
            <pre className="text-xs text-warm-300 overflow-auto max-h-96 bg-warm-900 rounded p-3">
        ...
        <div className="bg-warm-800 rounded-lg p-4">
          <h4 className="text-sm font-medium text-warm-300 mb-3">{t('apiDocs.endpoints')}</h4>
          <div className="space-y-2">
            {viewingSpec.endpoints.map((ep, i) => (
              <div key={i} className="flex items-center gap-3 bg-warm-900 rounded p-3">
                <span className={`px-2 py-1 rounded text-xs font-bold text-white ${METHOD_COLORS[ep.method] || 'bg-warm-600'}`}>
                  {ep.method}
                </span>
                <span className="text-warm-300 font-mono text-sm">{ep.path}</span>
                <span className="text-warm-500 text-sm">{ep.summary}</span>
              </div>
            ))}
          </div>
        </div>
  ...
  if (editingDoc) {
    return (
      <div className="space-y-6">
        <div className="flex items-center gap-3">
          <button onClick={() => setEditingDoc(null)} className="text-warm-400 hover:text-white text-sm">
            {t('apiDocs.backToList')}
          </button>
        ...
        <div className="space-y-4">
          <input
            ...
            className="w-full bg-warm-800 border border-warm-700 rounded px-3 py-2 text-white placeholder-warm-500"
          />
          <input
            value={description}
        ...
        <div>
          <div className="flex justify-between items-center mb-3">
            <h4 className="text-sm font-medium text-warm-300">{t('apiDocs.endpoints')}</h4>
          ...
          <div className="space-y-3">
            {endpoints.map((ep, i) => (
              <div key={i} className="bg-warm-800 rounded-lg p-3 space-y-2">
                <div className="flex gap-2">
                  <select
                    value={ep.method}
                    onChange={e => updateEndpoint(i, 'method', e.target.value)}
                    className="bg-warm-700 border border-warm-600 rounded px-2 py-1 text-white text-sm"
                  >
                    {HTTP_METHODS.map(m => (
                  ...
                  <input
                    ...
                    className="flex-1 bg-warm-700 border border-warm-600 rounded px-2 py-1 text-white text-sm font-mono"
                  />
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 bg-warm-700 border border-warm-600 rounded px-2 py-1 text-white text-sm"
                  />
                  <input
                    value={ep.summary || ''}
                  ...
                  <input
                    ...
                    className="w-24 bg-warm-700 border border-warm-600 rounded px-2 py-1 text-white text-sm"
                  />
                </div>
              </div>
            ))}
        ...
        <div>
          <h4 className="text-sm font-medium text-warm-300 mb-3">{t('apiDocs.sdkLanguages')}</h4>
        ...
        <div className="flex gap-3">
          <button onClick={() => setEditingDoc(null)} className="px-4 py-2 bg-warm-700 hover:bg-warm-600 text-white rounded">
            {t('apiDocs.cancel')}
          </button>
          <button
            onClick={handleSave}
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-warm-400 text-sm mt-1">{t('apiDocs.subtitle')}</p>
        </div>
      ...
      {docs.length === 0 ? (
        ...
        <div className="space-y-3">
          {docs.map(doc => (
            <div key={doc.id} className="bg-warm-800 rounded-lg p-4">
              <div className="flex justify-between items-start">
                <div className="flex-1">
                  <h4 className="font-medium text-white">{doc.projectName}</h4>
                  {doc.description && <p className="text-warm-400 text-sm mt-1">{doc.description}</p>}
                  <div className="flex gap-2 mt-2">
                    ...
                    <span className="text-warm-500 text-xs">{doc.endpoints.length} {t('apiDocs.endpointCount')}</span>
                ...
                <div className="flex gap-2">
                  ...
                  <button onClick={() => startEdit(doc)} className="px-3 py-1 bg-warm-700 hover:bg-warm-600 text-warm-300 rounded text-xs">
                    {t('apiDocs.edit')}
                  </button>
                  <button
