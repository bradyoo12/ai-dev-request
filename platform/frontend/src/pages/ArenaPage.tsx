  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-400 text-sm mt-1">{t('arena.subtitle', 'Compare AI models side-by-side and track which performs best for your tasks')}</p>
      </div>
      ...
      {tab === 'compare' && (
        <div className="space-y-4">
          <div className="bg-gray-800 rounded-lg p-6 space-y-4">
            ...
            <div>
              <label className="block text-sm text-gray-400 mb-1">{t('arena.taskCategory', 'Task Category')}</label>
              <select
                value={taskCategory}
            ...
            <div>
              <label className="block text-sm text-gray-400 mb-1">{t('arena.prompt', 'Prompt')}</label>
              <textarea
                value={prompt}
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                {outputs.map((output, idx) => (
                  <div
                    key={idx}
                    className={`bg-gray-800 rounded-lg border-t-4 ${MODEL_HEADER_COLORS[output.model] || 'border-gray-600'} ${
                      currentComparison.selectedModel === output.model ? 'ring-2 ring-green-500' : ''
                    }`}
                  >
                    <div className="p-4">
                      <div className="flex items-center justify-between mb-3">
                        <span className={`text-xs px-2 py-1 rounded ${MODEL_COLORS[output.model] || 'bg-gray-700 text-gray-300'}`}>
                          {output.model}
                        </span>
                        <span className="text-xs text-gray-500">{output.provider}</span>
                      </div>
                      <div className="grid grid-cols-3 gap-2 mb-3 text-center">
                        <div>
                          <p className="text-xs text-gray-500">{t('arena.latency', 'Latency')}</p>
                          <p className="text-sm text-white font-medium">{output.latencyMs}ms</p>
                        </div>
                        <div>
                          <p className="text-xs text-gray-500">{t('arena.tokens', 'Tokens')}</p>
                          <p className="text-sm text-white font-medium">{output.tokenCount}</p>
                        </div>
                        <div>
                          <p className="text-xs text-gray-500">{t('arena.cost', 'Cost')}</p>
                          <p className="text-sm text-white font-medium">${output.cost.toFixed(4)}</p>
                        </div>
                      </div>
                      <div className="bg-gray-900 rounded p-3 mb-3 max-h-64 overflow-auto">
                        <pre className="text-xs text-gray-300 whitespace-pre-wrap font-mono">{output.output}</pre>
                      </div>
                      {currentComparison.status !== 'winner_selected' && (
                        <button
                          onClick={() => handleSelectWinner(output.model)}
                          className="w-full py-2 bg-gray-700 text-white rounded-md text-sm hover:bg-green-700 transition-colors"
                        >
      ...
      {tab === 'history' && (
        <div className="space-y-4">
          {loading ? (
            <p className="text-gray-400 text-sm">{t('arena.loading', 'Loading...')}</p>
          ) : comparisons.length === 0 ? (
            <div className="bg-gray-800 rounded-lg p-8 text-center">
              <p className="text-gray-400">{t('arena.noHistory', 'No comparisons yet. Run your first arena comparison!')}</p>
            </div>
          ) : (
                  ...
                  <div className="flex justify-between items-start gap-4">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        <span className="text-xs px-2 py-0.5 bg-gray-700 text-gray-300 rounded">{c.taskCategory}</span>
      ...
      {tab === 'leaderboard' && (
        <div className="space-y-4">
          {loading ? (
            ...
            <div className="bg-gray-800 rounded-lg p-8 text-center">
              <p className="text-gray-400">{t('arena.noLeaderboard', 'No winners selected yet. Run comparisons and pick winners to build the leaderboard.')}</p>
            </div>
          ) : (
            leaderboard.map(entry => (
              <div key={entry.category} className="bg-gray-800 rounded-lg p-4">
                <div className="flex items-center justify-between mb-3">
                  <h4 className="text-white font-medium capitalize">{entry.category.replace(/-/g, ' ')}</h4>
                  <span className="text-xs text-gray-500">{entry.totalComparisons} {t('arena.comparisons', 'comparisons')}</span>
                </div>
                <div className="space-y-2">
                  {entry.models.map((model, idx) => (
                    <div key={model.model} className="flex items-center gap-3">
                      <span className="text-xs text-gray-500 w-4">{idx + 1}.</span>
                      <span className={`text-xs px-2 py-0.5 rounded ${MODEL_COLORS[model.model] || 'bg-gray-700 text-gray-300'}`}>
                        {model.model}
                      </span>
                      <div className="flex-1 bg-gray-900 rounded-full h-3 overflow-hidden">
                        <div
                          className={`h-full rounded-full transition-all ${
                            idx === 0 ? 'bg-green-500' : idx === 1 ? 'bg-yellow-500' : 'bg-gray-600'
                          }`}
                          style={{ width: `${model.winRate}%` }}
                        />
                      </div>
                      <span className="text-sm text-white font-medium w-14 text-right">{model.winRate}%</span>
                      <span className="text-xs text-gray-500 w-12 text-right">{model.wins} {t('arena.wins', 'wins')}</span>
                    </div>
                  ))}
                </div>
              </div>
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-3 gap-4">
            <div className="bg-gray-800 rounded-lg p-4 text-center">
              ...
              <p className="text-gray-400 text-sm">{t('arena.stats.totalComparisons', 'Total Comparisons')}</p>
            </div>
            <div className="bg-gray-800 rounded-lg p-4 text-center">
              <p className="text-2xl font-bold text-green-400">{stats.winnersSelected}</p>
              <p className="text-gray-400 text-sm">{t('arena.stats.winnersSelected', 'Winners Selected')}</p>
            </div>
            <div className="bg-gray-800 rounded-lg p-4 text-center">
              <p className="text-2xl font-bold text-blue-400">${stats.avgCost.toFixed(4)}</p>
              <p className="text-gray-400 text-sm">{t('arena.stats.avgCost', 'Avg Cost')}</p>
            </div>
          </div>
          <div className="grid grid-cols-3 gap-4">
            <div className="bg-gray-800 rounded-lg p-4 text-center">
              <p className="text-2xl font-bold text-purple-400">{stats.totalTokens.toLocaleString()}</p>
              <p className="text-gray-400 text-sm">{t('arena.stats.totalTokens', 'Total Tokens')}</p>
            </div>
            <div className="bg-gray-800 rounded-lg p-4 text-center">
              <p className="text-2xl font-bold text-yellow-400">{stats.fastestModel}</p>
              <p className="text-gray-400 text-sm">{t('arena.stats.fastestModel', 'Fastest Model')}</p>
            </div>
            <div className="bg-gray-800 rounded-lg p-4 text-center">
              <p className="text-2xl font-bold text-orange-400">{stats.mostSelected}</p>
              <p className="text-gray-400 text-sm">{t('arena.stats.mostSelected', 'Most Selected')}</p>
            </div>
          </div>
              ...
              <div className="space-y-2">
                {stats.winRates.map(wr => (
                  <div key={wr.model} className="flex items-center gap-3">
                    <span className={`text-xs px-2 py-0.5 rounded ${MODEL_COLORS[wr.model] || 'bg-gray-700 text-gray-300'}`}>
                      {wr.model}
                    </span>
                    <div className="flex-1 bg-gray-900 rounded-full h-3 overflow-hidden">
                      <div
                    ...
                    <span className="text-xs text-gray-500">({wr.wins} {t('arena.wins', 'wins')})</span>
                  </div>
                ))}
              </div>
            </div>
              ...
              <div className="space-y-2">
                {stats.recentComparisons.map((c, i) => (
                  <div key={i} className="flex items-center justify-between text-sm">
                    <span className="text-gray-300 truncate flex-1">{c.promptText}</span>
                    <span className="text-gray-400 ml-2">{c.taskCategory}</span>
                    <span className={`ml-2 text-xs px-2 py-0.5 rounded ${c.selectedModel ? 'bg-green-900 text-green-300' : 'bg-gray-700 text-gray-400'}`}>
