...
export default function BackgroundAgentPage() {
  ...
  const statusColor = (status: string) => {
    switch (status) {
      ...
      case 'completed': return 'bg-warm-100 text-warm-800'
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-warm-500 mt-1">{t('backgroundAgent.description')}</p>
      </div>
            ...
            <>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="bg-white border rounded-lg p-4">
                  ...
                  <div className="text-sm text-warm-500">{t('backgroundAgent.stats.total')}</div>
                </div>
                <div className="bg-white border rounded-lg p-4">
                  <div className="text-2xl font-bold text-green-600">{stats.activeAgents}</div>
                  <div className="text-sm text-warm-500">{t('backgroundAgent.stats.active')}</div>
                </div>
                <div className="bg-white border rounded-lg p-4">
                  <div className="text-2xl font-bold text-blue-600">{stats.completedAgents}</div>
                  <div className="text-sm text-warm-500">{t('backgroundAgent.stats.completed')}</div>
                </div>
                <div className="bg-white border rounded-lg p-4">
                  <div className="text-2xl font-bold text-red-600">{stats.failedAgents}</div>
                  <div className="text-sm text-warm-500">{t('backgroundAgent.stats.failed')}</div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="bg-white border rounded-lg p-4">
                  ...
                  <div className="text-sm text-warm-500">{t('backgroundAgent.stats.tokens')}</div>
                </div>
                <div className="bg-white border rounded-lg p-4">
                  <div className="text-lg font-semibold">${stats.totalEstimatedCost.toFixed(2)}</div>
                  <div className="text-sm text-warm-500">{t('backgroundAgent.stats.cost')}</div>
                </div>
                <div className="bg-white border rounded-lg p-4">
                  <div className="text-lg font-semibold">{stats.totalPullRequests}</div>
                  <div className="text-sm text-warm-500">{t('backgroundAgent.stats.pullRequests')}</div>
                </div>
                <div className="bg-white border rounded-lg p-4">
                  <div className="text-lg font-semibold">{formatSeconds(Math.round(stats.avgCompletionSeconds))}</div>
                  <div className="text-sm text-warm-500">{t('backgroundAgent.stats.avgTime')}</div>
                </div>
              </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  {agentTypes.map((at) => (
                    <div key={at.type} className="bg-white border rounded-lg p-4">
                      ...
                      <div className="text-sm text-warm-500 mt-1">{at.description}</div>
                    </div>
                  ))}
                </div>
              </div>
      ...
      {tab === 'agents' && (
        <div className="space-y-4">
          <div className="flex gap-2 items-center">
            ...
            <button onClick={loadAgents} className="px-3 py-1.5 bg-warm-100 rounded text-sm hover:bg-warm-200">
              {t('backgroundAgent.refresh')}
            </button>
          </div>

          {loading ? (
            <div className="text-warm-500 text-sm">{t('backgroundAgent.loading')}</div>
          ) : agents.length === 0 ? (
            <div className="text-warm-400 text-sm py-8 text-center">{t('backgroundAgent.noAgents')}</div>
          ) : (
              ...
              {agents.map((agent) => (
                ...
                >
                  ...
                  {agent.taskDescription && (
                    <div className="text-sm text-warm-500 mt-1 truncate">{agent.taskDescription}</div>
                  )}
                  <div className="mt-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-warm-500">{agent.progressPercent}%</span>
                    </div>
          ...
          {selectedAgent && (
            <div className="bg-white border rounded-lg p-6 mt-4">
              <div className="flex items-center justify-between mb-4">
                ...
                <button onClick={() => setSelectedAgent(null)} className="text-warm-400 hover:text-warm-600">x</button>
              </div>

              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
                <div className="text-center">
                  <div className="text-lg font-bold">{selectedAgent.filesCreated}</div>
                  <div className="text-xs text-warm-500">{t('backgroundAgent.detail.filesCreated')}</div>
                </div>
                <div className="text-center">
                  <div className="text-lg font-bold">{selectedAgent.filesModified}</div>
                  <div className="text-xs text-warm-500">{t('backgroundAgent.detail.filesModified')}</div>
                </div>
                <div className="text-center">
                  <div className="text-lg font-bold text-green-600">{selectedAgent.testsPassed}</div>
                  <div className="text-xs text-warm-500">{t('backgroundAgent.detail.testsPassed')}</div>
                </div>
                <div className="text-center">
                  <div className="text-lg font-bold text-red-600">{selectedAgent.testsFailed}</div>
                  <div className="text-xs text-warm-500">{t('backgroundAgent.detail.testsFailed')}</div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
                <div className="text-center">
                  ...
                  <div className="text-xs text-warm-500">{t('backgroundAgent.detail.cpu')}</div>
                </div>
                <div className="text-center">
                  <div className="text-sm font-semibold">{selectedAgent.memoryUsageMb.toFixed(0)} MB</div>
                  <div className="text-xs text-warm-500">{t('backgroundAgent.detail.memory')}</div>
                </div>
                <div className="text-center">
                  <div className="text-sm font-semibold">{selectedAgent.tokensUsed.toLocaleString()}</div>
                  <div className="text-xs text-warm-500">{t('backgroundAgent.detail.tokens')}</div>
                </div>
                <div className="text-center">
                  <div className="text-sm font-semibold">${selectedAgent.estimatedCost.toFixed(3)}</div>
                  <div className="text-xs text-warm-500">{t('backgroundAgent.detail.cost')}</div>
                </div>
              </div>
                  ...
                  <div className="space-y-1">
                    {selectedAgent.steps.map((step, i) => (
                      <div key={i} className="flex items-center gap-2 text-sm">
                        <span className={`w-2 h-2 rounded-full ${step.status === 'completed' ? 'bg-green-500' : step.status === 'running' ? 'bg-blue-500 animate-pulse' : 'bg-warm-300'}`} />
                        <span className={step.status === 'completed' ? 'text-warm-600' : step.status === 'running' ? 'text-blue-700 font-medium' : 'text-warm-400'}>
                          {step.name}
                        </span>
                      </div>
                    ))}
      ...
      {tab === 'logs' && selectedAgent && (
        <div className="space-y-2">
          ...
          {selectedAgent.logEntries.length === 0 ? (
            <div className="text-warm-400 text-sm">{t('backgroundAgent.noLogs')}</div>
          ) : (
            <div className="bg-warm-900 text-warm-100 rounded-lg p-4 font-mono text-xs space-y-1 max-h-96 overflow-y-auto">
      ...
      {tab === 'logs' && !selectedAgent && (
        <div className="text-warm-400 text-sm py-8 text-center">{t('backgroundAgent.selectAgentForLogs')}</div>
      )}
    </div>
  )
}
