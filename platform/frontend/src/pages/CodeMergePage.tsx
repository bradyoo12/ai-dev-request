...
export default function CodeMergePage() {
  ...
  if (loading) {
    return <div className="text-gray-400 text-center py-12">{t('codeMerge.loading')}</div>
  }
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center gap-3">
          <button onClick={() => setSelectedSnapshot(null)} className="text-gray-400 hover:text-white text-sm">
            {t('codeMerge.backToList')}
          </button>
          <h3 className="text-xl font-bold">{selectedSnapshot.filePath}</h3>
          <span className={`px-2 py-1 rounded text-xs font-medium ${STATUS_COLORS[selectedSnapshot.status] || 'bg-gray-600/20 text-gray-400'}`}>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="bg-gray-800 rounded-lg p-4">
            ...
            <pre className="text-xs text-gray-300 overflow-auto max-h-80 bg-gray-900 rounded p-3 font-mono">
        ...
        {selectedSnapshot.status === 'Conflicted' && (
          <div className="bg-gray-800 rounded-lg p-4">
            <h4 className="text-sm font-medium text-gray-300 mb-3">{t('codeMerge.resolveConflict')}</h4>
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-400 text-sm mt-1">{t('codeMerge.subtitle')}</p>
      </div>
      ...
      {stats && (
        <div className="grid grid-cols-3 md:grid-cols-6 gap-3">
          <div className="bg-gray-800 rounded-lg p-3 text-center">
            ...
            <div className="text-xs text-gray-400">{t('codeMerge.statTotal')}</div>
          </div>
          <div className="bg-gray-800 rounded-lg p-3 text-center">
            <div className="text-2xl font-bold text-green-400">{stats.syncedFiles}</div>
            <div className="text-xs text-gray-400">{t('codeMerge.statSynced')}</div>
          </div>
          <div className="bg-gray-800 rounded-lg p-3 text-center">
            <div className="text-2xl font-bold text-yellow-400">{stats.modifiedFiles}</div>
            <div className="text-xs text-gray-400">{t('codeMerge.statModified')}</div>
          </div>
          <div className="bg-gray-800 rounded-lg p-3 text-center">
            <div className="text-2xl font-bold text-red-400">{stats.conflictedFiles}</div>
            <div className="text-xs text-gray-400">{t('codeMerge.statConflicted')}</div>
          </div>
          <div className="bg-gray-800 rounded-lg p-3 text-center">
            <div className="text-2xl font-bold text-blue-400">{stats.mergedFiles}</div>
            <div className="text-xs text-gray-400">{t('codeMerge.statMerged')}</div>
          </div>
          <div className="bg-gray-800 rounded-lg p-3 text-center">
            <div className="text-2xl font-bold text-purple-400">{stats.lockedFiles}</div>
            <div className="text-xs text-gray-400">{t('codeMerge.statLocked')}</div>
          </div>
        </div>
      )}
      ...
      {filteredSnapshots.length === 0 ? (
        ...
        <div className="space-y-2">
          {filteredSnapshots.map(snapshot => (
            <div key={snapshot.id} className="bg-gray-800 rounded-lg p-3 flex items-center justify-between">
              <div className="flex items-center gap-3 flex-1 min-w-0">
                <span className={`font-mono text-sm ${getFileColor(snapshot.filePath)}`}>
                  {snapshot.filePath}
                </span>
                <span className={`px-2 py-0.5 rounded text-xs font-medium shrink-0 ${STATUS_COLORS[snapshot.status] || 'bg-gray-600/20 text-gray-400'}`}>
                  {snapshot.status}
                </span>
                <span className="text-gray-500 text-xs shrink-0">v{snapshot.version}</span>
                {snapshot.isLocked && (
