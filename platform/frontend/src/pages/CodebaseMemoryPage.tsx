  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-warm-400 text-sm mt-1">{t('codebaseMemory.subtitle', 'Persistent learning from your codebase patterns and preferences')}</p>
      </div>
        ...
        <div className="space-y-4">
          <div className="flex gap-2 items-center">
            <select
              ...
              className="bg-warm-800 border border-warm-700 text-white rounded-md px-3 py-2 text-sm"
            >
            ...
            <span className="text-warm-400 text-sm">{memories.length} {t('codebaseMemory.memoriesFound', 'memories')}</span>
          </div>

          {loading ? (
            <p className="text-warm-400 text-sm">{t('codebaseMemory.loading', 'Loading...')}</p>
          ) : memories.length === 0 ? (
            <div className="bg-warm-800 rounded-lg p-8 text-center">
              <p className="text-warm-400">{t('codebaseMemory.noMemories', 'No memories yet. Add patterns and conventions the AI should remember.')}</p>
            </div>
          ) : (
            <div className="space-y-3">
              {memories.map(m => (
                <div key={m.id} className={`bg-warm-800 rounded-lg p-4 border ${m.isActive ? 'border-warm-700' : 'border-red-900 opacity-60'}`}>
                  <div className="flex justify-between items-start gap-4">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <span className="text-xs px-2 py-0.5 bg-blue-900 text-blue-300 rounded">{m.memoryType}</span>
                        <span className="text-xs px-2 py-0.5 bg-warm-700 text-warm-300 rounded">{m.category}</span>
                      ...
                      <p className="text-warm-300 text-sm mt-1 whitespace-pre-wrap">{m.content}</p>
      ...
      {tab === 'add' && (
        <div className="bg-warm-800 rounded-lg p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm text-warm-400 mb-1">{t('codebaseMemory.memoryType', 'Memory Type')}</label>
              <select value={memoryType} onChange={e => setMemoryType(e.target.value)} className="w-full bg-warm-700 border border-warm-600 text-white rounded-md px-3 py-2 text-sm">
                {MEMORY_TYPES.map(mt => <option key={mt} value={mt}>{mt}</option>)}
              </select>
            </div>
            <div>
              <label className="block text-sm text-warm-400 mb-1">{t('codebaseMemory.category', 'Category')}</label>
              <select value={category} onChange={e => setCategory(e.target.value)} className="w-full bg-warm-700 border border-warm-600 text-white rounded-md px-3 py-2 text-sm">
                {CATEGORIES.map(c => <option key={c} value={c}>{c}</option>)}
              </select>
            </div>
          </div>
          <div>
            <label className="block text-sm text-warm-400 mb-1">{t('codebaseMemory.summaryLabel', 'Summary')}</label>
            <input value={summary} onChange={e => setSummary(e.target.value)} className="w-full bg-warm-700 border border-warm-600 text-white rounded-md px-3 py-2 text-sm" placeholder={t('codebaseMemory.summaryPlaceholder', 'Brief description of this memory')} />
          </div>
          <div>
            <label className="block text-sm text-warm-400 mb-1">{t('codebaseMemory.contentLabel', 'Content')}</label>
            <textarea value={content} onChange={e => setContent(e.target.value)} rows={4} className="w-full bg-warm-700 border border-warm-600 text-white rounded-md px-3 py-2 text-sm" placeholder={t('codebaseMemory.contentPlaceholder', 'e.g., Use camelCase for variables, PascalCase for components...')} />
          </div>
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-3 gap-4">
            <div className="bg-warm-800 rounded-lg p-4 text-center">
              ...
              <p className="text-warm-400 text-sm">{t('codebaseMemory.stats.total', 'Total Memories')}</p>
            </div>
            <div className="bg-warm-800 rounded-lg p-4 text-center">
              <p className="text-2xl font-bold text-green-400">{stats.activeMemories}</p>
              <p className="text-warm-400 text-sm">{t('codebaseMemory.stats.active', 'Active')}</p>
            </div>
            <div className="bg-warm-800 rounded-lg p-4 text-center">
              <p className="text-2xl font-bold text-blue-400">{stats.avgConfidence}%</p>
              <p className="text-warm-400 text-sm">{t('codebaseMemory.stats.avgConfidence', 'Avg Confidence')}</p>
            </div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-warm-800 rounded-lg p-4 text-center">
              <p className="text-2xl font-bold text-green-400">{stats.totalReinforcements}</p>
              <p className="text-warm-400 text-sm">{t('codebaseMemory.stats.reinforcements', 'Reinforcements')}</p>
            </div>
            <div className="bg-warm-800 rounded-lg p-4 text-center">
              <p className="text-2xl font-bold text-yellow-400">{stats.totalContradictions}</p>
              <p className="text-warm-400 text-sm">{t('codebaseMemory.stats.contradictions', 'Contradictions')}</p>
            </div>
          </div>
              ...
              <div className="space-y-2">
                {stats.byCategory.map(c => (
                  <div key={c.category} className="flex items-center justify-between text-sm">
                    <span className="text-warm-300">{c.category}</span>
                    <div className="flex items-center gap-3">
                      <span className="text-warm-400">{c.count} memories</span>
              ...
              <div className="space-y-2">
                {stats.recentMemories.map((m, i) => (
                  <div key={i} className="flex items-center justify-between text-sm">
                    <span className="text-warm-300 truncate flex-1">{m.summary || 'Unnamed'}</span>
                    <span className="text-warm-400 ml-2">{m.category}</span>
