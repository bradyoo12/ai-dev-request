...
export default function CompliancePage() {
  ...
  if (!authUser) {
    return (
      <div className="max-w-4xl mx-auto text-center py-16">
        ...
        <p className="text-warm-400">{t('compliance.loginRequired', 'Please log in to view compliance data.')}</p>
      </div>
    )
  }
  ...
  if (!requestId) {
    return (
      <div className="max-w-4xl mx-auto text-center py-16">
        <h2 className="text-2xl font-bold mb-2">{t('compliance.title', 'Compliance Dashboard')}</h2>
        <p className="text-warm-400">{t('compliance.noProject', 'No project selected. Go to your project and run a security scan.')}</p>
        <button
          onClick={() => navigate('/')}
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-warm-400 mt-1">{t('compliance.description', 'SBOM, vulnerability scanning, and license compliance')}</p>
        </div>
        <button
          onClick={handleScan}
          disabled={scanning}
      ...
      {(scanResult || sbomReport) && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <div className="bg-warm-800 rounded-lg p-4 text-center">
            ...
            <div className="text-xs text-warm-400 mt-1">{t('compliance.dependencies', 'Dependencies')}</div>
          </div>
          <div className="bg-warm-800 rounded-lg p-4 text-center">
            <div className="text-2xl font-bold text-red-400">
              {vulnerabilities.filter(v => v.severity === 'critical').length}
            </div>
            <div className="text-xs text-warm-400 mt-1">{t('compliance.critical', 'Critical')}</div>
          </div>
          <div className="bg-warm-800 rounded-lg p-4 text-center">
            <div className="text-2xl font-bold text-orange-400">
              {vulnerabilities.filter(v => v.severity === 'high').length}
            </div>
            <div className="text-xs text-warm-400 mt-1">{t('compliance.high', 'High')}</div>
          </div>
          <div className="bg-warm-800 rounded-lg p-4 text-center">
            <div className="text-2xl font-bold text-yellow-400">
              {vulnerabilities.filter(v => v.severity === 'medium' || v.severity === 'low').length}
            </div>
            <div className="text-xs text-warm-400 mt-1">{t('compliance.mediumLow', 'Medium/Low')}</div>
          </div>
        </div>
      )}
      ...
      {loading && (
        <div className="text-center py-8 text-warm-400">{t('compliance.loading', 'Loading...')}</div>
      )}
        ...
        <div className="space-y-4">
          ...
          {components.length === 0 ? (
            <div className="text-center py-12 bg-warm-800/50 rounded-lg">
              <p className="text-warm-400">{t('compliance.noSbom', 'No SBOM data. Run a security scan to generate.')}</p>
            </div>
          ) : (
            <div className="space-y-1">
              ...
              {filteredComponents.map((comp, i) => (
                <div key={i} className="grid grid-cols-12 gap-2 bg-warm-800 rounded-lg px-3 py-2 text-sm">
                  ...
                  <div className="col-span-2 text-warm-400 font-mono text-xs">{comp.version}</div>
                  <div className="col-span-2">
                    <span className="px-1.5 py-0.5 rounded text-xs bg-warm-700 text-warm-300">{comp.ecosystem}</span>
                  </div>
                  <div className="col-span-2 text-warm-400 text-xs">{comp.type}</div>
                  <div className="col-span-2 text-warm-400 text-xs">{comp.license || '-'}</div>
                </div>
              ))}
        ...
        <div className="space-y-3">
          {vulnerabilities.length === 0 ? (
            ...
            vulnerabilities.map((vuln) => (
              <div key={vuln.id} className="bg-warm-800 rounded-lg p-4">
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="text-xs text-warm-500 font-mono">{vuln.vulnerabilityId}</span>
                    </div>
                    <h4 className="text-sm font-medium text-white">
                      {vuln.packageName}
                      <span className="text-warm-500 font-mono ml-1">@{vuln.packageVersion}</span>
                    </h4>
                    <p className="text-xs text-warm-400 mt-1">{vuln.summary}</p>
                    {vuln.fixedVersion && (
                      <p className="text-xs mt-1">
                        <span className="text-warm-500">{t('compliance.fixAvailable', 'Fix available')}:</span>{' '}
      ...
      {!loading && activeTab === 'licenses' && (
        <div className="space-y-4">
          {!licenseAnalysis ? (
            <div className="text-center py-12 bg-warm-800/50 rounded-lg">
              <p className="text-warm-400">{t('compliance.noLicenseData', 'No license data. Run a security scan first.')}</p>
            </div>
          ) : (
            <>
              ...
              <div className="space-y-2">
                {licenseAnalysis.licenseGroups.map((group) => (
                  <div key={group.license} className="bg-warm-800 rounded-lg p-4">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className="text-sm text-warm-400">{group.count} {t('compliance.packages', 'packages')}</span>
                    </div>
                    <div className="flex flex-wrap gap-1 mt-1">
                      {group.packages.slice(0, 10).map((pkg) => (
                        <span key={pkg} className="px-1.5 py-0.5 rounded text-xs bg-warm-700 text-warm-300">
                          {pkg}
                        </span>
                      ))}
                      {group.packages.length > 10 && (
                        <span className="px-1.5 py-0.5 rounded text-xs bg-warm-700 text-warm-500">
