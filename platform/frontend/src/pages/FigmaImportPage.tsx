  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-warm-400 text-sm mt-1">{t('figmaImport.subtitle', 'Import Figma designs and generate production-ready components')}</p>
      </div>
      ...
      {tab === 'import' && (
        <div className="space-y-4">
          <div className="bg-warm-800 rounded-lg p-6 space-y-4">
            ...
            <div>
              <label className="block text-sm text-warm-400 mb-1">{t('figmaImport.figmaUrl', 'Figma URL')}</label>
              <input value={figmaUrl} onChange={e => setFigmaUrl(e.target.value)} className="w-full bg-warm-700 border border-warm-600 text-white rounded-md px-3 py-2 text-sm" placeholder="https://www.figma.com/file/..." />
            </div>
            <div>
              <label className="block text-sm text-warm-400 mb-1">{t('figmaImport.designName', 'Design Name')}</label>
              <input value={designName} onChange={e => setDesignName(e.target.value)} className="w-full bg-warm-700 border border-warm-600 text-white rounded-md px-3 py-2 text-sm" placeholder={t('figmaImport.namePlaceholder', 'Optional name for this import')} />
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm text-warm-400 mb-1">{t('figmaImport.framework', 'Framework')}</label>
              ...
              <div>
                <label className="block text-sm text-warm-400 mb-1">{t('figmaImport.styling', 'Styling')}</label>
            ...
            <div className="flex gap-2">
              ...
              <button onClick={handleImportScreenshot} disabled={importing} className="px-4 py-2 bg-warm-700 text-white rounded-md text-sm hover:bg-warm-600 disabled:opacity-50">
            ...
            <div className="bg-warm-800 rounded-lg p-6 space-y-4 border border-purple-700">
              ...
              <div className="grid grid-cols-3 gap-4">
                <div className="text-center">
                  ...
                  <p className="text-warm-400 text-xs">{t('figmaImport.components', 'Components')}</p>
                </div>
                <div className="text-center">
                  <p className="text-lg font-bold text-blue-400">{selectedImport.tokenCount}</p>
                  <p className="text-warm-400 text-xs">{t('figmaImport.tokens', 'Design Tokens')}</p>
                </div>
                <div className="text-center">
                  <p className="text-lg font-bold text-green-400">{Math.round(selectedImport.processingTimeMs)}ms</p>
                  <p className="text-warm-400 text-xs">{t('figmaImport.processingTime', 'Processing')}</p>
                </div>
              </div>
              <div>
                <h5 className="text-warm-400 text-sm mb-2">{t('figmaImport.extractedTokens', 'Extracted Tokens')}</h5>
                <pre className="bg-warm-900 p-3 rounded text-xs text-warm-300 overflow-x-auto max-h-40">{JSON.stringify(JSON.parse(selectedImport.designTokensJson), null, 2)}</pre>
              </div>
              <div>
                <h5 className="text-warm-400 text-sm mb-2">{t('figmaImport.generatedCode', 'Generated Code')}</h5>
                <pre className="bg-warm-900 p-3 rounded text-xs text-warm-300 overflow-x-auto max-h-40">{JSON.stringify(JSON.parse(selectedImport.generatedCodeJson), null, 2)}</pre>
              </div>
            </div>
          )}
        </div>
      ...
      {tab === 'history' && (
        <div className="space-y-3">
          {loading ? (
            <p className="text-warm-400 text-sm">{t('figmaImport.loading', 'Loading...')}</p>
          ) : imports.length === 0 ? (
            <div className="bg-warm-800 rounded-lg p-8 text-center">
              <p className="text-warm-400">{t('figmaImport.noImports', 'No imports yet. Import a Figma design to get started.')}</p>
            </div>
          ) : (
            imports.map(imp => (
              <div key={imp.id} className="bg-warm-800 rounded-lg p-4 flex justify-between items-center">
                <div>
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-3 gap-4">
            <div className="bg-warm-800 rounded-lg p-4 text-center">
              ...
              <p className="text-warm-400 text-sm">{t('figmaImport.stats.total', 'Total Imports')}</p>
            </div>
            <div className="bg-warm-800 rounded-lg p-4 text-center">
              <p className="text-2xl font-bold text-purple-400">{stats.totalComponents}</p>
              <p className="text-warm-400 text-sm">{t('figmaImport.stats.components', 'Components')}</p>
            </div>
            <div className="bg-warm-800 rounded-lg p-4 text-center">
              <p className="text-2xl font-bold text-blue-400">{stats.totalTokens}</p>
              <p className="text-warm-400 text-sm">{t('figmaImport.stats.tokens', 'Design Tokens')}</p>
            </div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-warm-800 rounded-lg p-4 text-center">
              <p className="text-2xl font-bold text-green-400">{stats.completedImports}</p>
              <p className="text-warm-400 text-sm">{t('figmaImport.stats.completed', 'Completed')}</p>
            </div>
            <div className="bg-warm-800 rounded-lg p-4 text-center">
              <p className="text-2xl font-bold text-yellow-400">{stats.avgProcessingTime}ms</p>
              <p className="text-warm-400 text-sm">{t('figmaImport.stats.avgTime', 'Avg Processing')}</p>
            </div>
          </div>
                ...
                {stats.recentImports.map((r, i) => (
                  <div key={i} className="flex items-center justify-between text-sm">
                    ...
                    <div className="flex items-center gap-2">
                      <span className="text-warm-400">{r.sourceType}</span>
