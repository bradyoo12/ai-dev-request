...
export default function InfrastructurePage() {
  ...
  if (!authUser) {
    return (
      <div className="max-w-4xl mx-auto text-center py-16">
        ...
        <p className="text-warm-400">{t('infrastructure.loginRequired', 'Please log in to manage infrastructure.')}</p>
      </div>
    )
  }
  ...
  if (!requestId) {
    return (
      <div className="max-w-4xl mx-auto text-center py-16">
        <h2 className="text-2xl font-bold mb-2">{t('infrastructure.title', 'Infrastructure')}</h2>
        <p className="text-warm-400">{t('infrastructure.noProject', 'No project selected. Go to your project to configure infrastructure.')}</p>
        <button
          onClick={() => navigate('/')}
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-warm-400 mt-1">{t('infrastructure.description', 'Configure and generate Azure Bicep templates for your project')}</p>
        </div>
        {!config && (
          <button
      ...
      {loading && (
        <div className="text-center py-8 text-warm-400">{t('infrastructure.loading', 'Loading...')}</div>
      )}
      ...
      {config?.analysisSummary && (
        <div className="bg-blue-900/20 border border-blue-800 rounded-lg p-4">
          ...
          <pre className="text-xs text-warm-300 whitespace-pre-wrap">{config.analysisSummary}</pre>
        </div>
      )}
                ...
                <label
                  ...
                  className={`flex items-start gap-3 p-3 rounded-lg cursor-pointer transition-colors ${
                    selectedServices.includes(service.id)
                      ...
                      : 'bg-warm-900 border border-warm-700 hover:border-warm-600'
                  }`}
                >
                  <input
                    type="checkbox"
                    ...
                    className="mt-1 rounded border-warm-600 text-blue-600 focus:ring-blue-500"
                  />
                  <div>
                    <div className="text-sm font-medium text-white">{service.label}</div>
                    <div className="text-xs text-warm-400">{service.description}</div>
                  </div>
                </label>
              ))}
            </div>
            ...
            <div className="grid grid-cols-3 gap-3">
              {TIERS.map((tier) => (
                ...
                >
                  ...
                  <div className="text-xs mt-1 text-warm-500">{tier.description}</div>
                </button>
              ))}
            </div>
          </div>
            ...
            <div className="bg-warm-800 rounded-xl p-6">
              ...
              <div className="space-y-2">
                {costEstimation.lineItems.map((item) => (
                  <div key={item.service} className="flex items-center justify-between py-2 border-b border-warm-700/50">
                    <span className="text-sm text-warm-300">{item.displayName}</span>
        ...
        <div className="space-y-4">
          {!config.generatedBicepMain ? (
            <div className="text-center py-12 bg-warm-800/50 rounded-lg">
              <p className="text-warm-400">{t('infrastructure.noTemplates', 'No templates generated yet. Go to Configuration and click Generate.')}</p>
            </div>
          ) : (
            <>
              ...
              <div className="bg-warm-900 rounded-lg p-4 overflow-auto max-h-[600px]">
                <div className="text-xs text-warm-500 mb-2 font-mono">main.bicep</div>
                <pre className="text-xs text-warm-300 font-mono whitespace-pre">{config.generatedBicepMain}</pre>
              </div>

              {config.generatedBicepParameters && (
                <div className="bg-warm-900 rounded-lg p-4 overflow-auto max-h-[200px]">
                  <div className="text-xs text-warm-500 mb-2 font-mono">main.bicepparam</div>
                  <pre className="text-xs text-warm-300 font-mono whitespace-pre">{config.generatedBicepParameters}</pre>
                </div>
              )}
            </>
          )}
      ...
      {config && activeTab === 'deploy' && (
        <div className="space-y-4">
          <div className="bg-warm-800 rounded-xl p-6">
            ...
            <div className="space-y-4">
              <div>
                <p className="text-sm text-warm-400 mb-2">{t('infrastructure.step1', '1. Install Azure Developer CLI (azd)')}</p>
              ...
              <div>
                <p className="text-sm text-warm-400 mb-2">{t('infrastructure.step2', '2. Download and extract the templates')}</p>
                <button
                  onClick={handleDownload}
              ...
              <div>
                <p className="text-sm text-warm-400 mb-2">{t('infrastructure.step3', '3. Initialize and deploy')}</p>
                <div className="bg-warm-900 rounded-lg p-3 space-y-1">
              ...
              <div>
                <p className="text-sm text-warm-400 mb-2">{t('infrastructure.stepAlt', 'Alternative: Deploy with Azure CLI')}</p>
                <div className="bg-warm-900 rounded-lg p-3 space-y-1">
