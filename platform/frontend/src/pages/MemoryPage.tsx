...
export default function MemoryPage() {
  ...
  if (loading && memories.length === 0) {
    return <div className="text-center py-8 text-warm-400">{t('memory.loading')}</div>
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-warm-400 mt-1">{t('memory.description')}</p>
        </div>
      ...
      {memories.length === 0 ? (
        <div className="text-center py-12 bg-warm-800/50 rounded-lg">
          <p className="text-warm-400">{t('memory.empty')}</p>
        </div>
      ) : (
        <>
          <div className="text-sm text-warm-400">{t('memory.totalCount', { count: totalCount })}</div>
          <div className="space-y-3">
            {memories.map((m) => (
              <div key={m.id} className="bg-warm-800 rounded-lg p-4 flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-3 mt-2">
                    ...
                    <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-warm-700 text-warm-300">
                      {m.scope}
                    </span>
            ...
            <div className="space-y-4">
              <div>
                ...
                <textarea
                  ...
                  className="w-full bg-warm-700 border border-warm-600 rounded-lg p-3 text-white text-sm resize-none"
                  rows={3}
                  placeholder={t('memory.contentPlaceholder')}
                />
              </div>
              <div>
                <label className="block text-sm text-warm-400 mb-1">{t('memory.categoryLabel')}</label>
                <select
                  value={newCategory}
