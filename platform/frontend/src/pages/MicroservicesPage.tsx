  ...
  if (!user) {
    return (
      <div className="max-w-4xl mx-auto text-center py-16">
        ...
        <p className="text-gray-400">{t('micro.loginRequired')}</p>
      </div>
    )
  }

  if (loading) {
    return <div className="max-w-4xl mx-auto text-center py-8 text-gray-400">{t('micro.loading')}</div>
  }
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">{t('micro.description')}</p>
        </div>
      </div>
        ...
        <div className="space-y-3">
          {blueprints.length === 0 ? (
            <div className="text-center py-12 bg-gray-800/50 rounded-lg">
              <p className="text-gray-400">{t('micro.empty')}</p>
              <p className="text-sm text-gray-500 mt-1">{t('micro.emptyHint')}</p>
            </div>
          ) : (
            blueprints.map((bp) => (
              <button
        ...
        <div className="space-y-4">
          ...
          {activeTab === 'services' && (
            <div className="grid gap-3 md:grid-cols-2">
              {services.map((svc) => (
                <div key={svc.Name} className="bg-gray-800 rounded-lg p-4">
                  <div className="flex items-center gap-2 mb-2">
                    <span className={`px-2 py-0.5 rounded text-xs font-medium ${TYPE_COLORS[svc.Type] || 'bg-gray-700 text-gray-400'}`}>
                      {svc.Type}
                    </span>
                    <span className="text-xs text-gray-500">{svc.Tech}</span>
                  ...
                  {svc.Endpoints.length > 0 && (
                    <div className="mt-2 space-y-0.5">
                      {svc.Endpoints.map((ep, i) => (
                        <code key={i} className="block text-xs text-gray-500 font-mono">{ep}</code>
                      ))}
                    </div>
                  )}
                </div>
            ...
            <div className="space-y-2">
              {dependencies.map((dep, i) => (
                <div key={i} className="bg-gray-800 rounded-lg p-3 flex items-center gap-3">
                  ...
                  <span className="px-2 py-0.5 text-xs bg-gray-700 text-gray-300 rounded">{dep.Protocol}</span>
                  <span className="text-xs text-gray-500 flex-1 truncate">{dep.Description}</span>
                </div>
              ))}
            </div>
          )}
          ...
          {activeTab === 'docker' && (
            <div className="bg-gray-800 rounded-lg p-4">
              ...
              <pre className="text-xs text-gray-300 font-mono overflow-x-auto whitespace-pre max-h-96 overflow-y-auto">
