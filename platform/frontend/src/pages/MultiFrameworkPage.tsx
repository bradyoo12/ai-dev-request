...
export default function MultiFrameworkPage() {
  ...
  return (
    <div>
      ...
      <p className="text-gray-400 text-sm mb-6">{t('multiFramework.subtitle', 'Choose your preferred tech stack for AI-generated projects')}</p>
      ...
      {tab === 'frameworks' && (
        <div className="space-y-4">
          <div className="flex gap-2">
            <button onClick={() => setFilterTier(null)} className={`px-3 py-1 rounded-full text-xs font-medium ${!filterTier ? 'bg-blue-600 text-white' : 'bg-gray-800 text-gray-400'}`}>
              {t('multiFramework.allTiers', 'All Tiers')}
            </button>
            {[1, 2, 3].map((tier) => (
              <button key={tier} onClick={() => setFilterTier(tier)} className={`px-3 py-1 rounded-full text-xs font-medium ${filterTier === tier ? 'bg-blue-600 text-white' : 'bg-gray-800 text-gray-400'}`}>
            ...
            {filteredFrameworks.map((fw) => (
              <div
                ...
                className={`bg-gray-800 border rounded-lg p-4 cursor-pointer transition-all ${
                  config?.selectedFramework === fw.id ? 'border-blue-500 ring-1 ring-blue-500' : 'border-gray-700 hover:border-gray-500'
                }`}
              >
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="flex gap-1">
                    ...
                    <span className={`text-xs px-2 py-0.5 rounded ${categoryColors[fw.category] || 'bg-gray-700 text-gray-400'}`}>{fw.category}</span>
                  </div>
                </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              {backends.map((b) => (
                <button
                  ...
                  className={`p-3 rounded-lg border text-left ${config.selectedBackend === b.id ? 'border-blue-500 bg-blue-900/20' : 'border-gray-700 bg-gray-800'}`}
                >
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              {databases.map((d) => (
                <button
                  ...
                  className={`p-3 rounded-lg border text-left ${config.selectedDatabase === d.id ? 'border-blue-500 bg-blue-900/20' : 'border-gray-700 bg-gray-800'}`}
                >
      ...
      {tab === 'preview' && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <div className="text-sm text-gray-400">{t('multiFramework.currentStack', 'Current Stack')}</div>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-gray-800 border border-gray-700 rounded-lg p-3 text-center">
                    ...
                    <div className="text-xs text-gray-400">{t('multiFramework.tokens', 'Tokens')}</div>
                  </div>
                  <div className="bg-gray-800 border border-gray-700 rounded-lg p-3 text-center">
                    <div className="text-xl font-bold">${preview.estimatedCost}</div>
                    <div className="text-xs text-gray-400">{t('multiFramework.cost', 'Cost')}</div>
                  </div>
                  <div className="bg-gray-800 border border-gray-700 rounded-lg p-3 text-center">
                    <div className="text-xl font-bold">{preview.estimatedTimeSeconds}s</div>
                    <div className="text-xs text-gray-400">{t('multiFramework.time', 'Time')}</div>
                  </div>
                  <div className="bg-gray-800 border border-gray-700 rounded-lg p-3 text-center">
                    <div className="text-xl font-bold">{preview.estimatedFiles}</div>
                    <div className="text-xs text-gray-400">{t('multiFramework.files', 'Files')}</div>
                  </div>
                </div>
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            <div className="bg-gray-800 border border-gray-700 rounded-lg p-4 text-center">
              ...
              <div className="text-sm text-gray-400">{t('multiFramework.stats.totalProjects', 'Total Projects')}</div>
            </div>
            <div className="bg-gray-800 border border-gray-700 rounded-lg p-4 text-center">
              <div className="text-2xl font-bold">{stats.frameworksUsed}</div>
              <div className="text-sm text-gray-400">{t('multiFramework.stats.frameworksUsed', 'Frameworks Used')}</div>
            </div>
            <div className="bg-gray-800 border border-gray-700 rounded-lg p-4 text-center">
              <div className="text-2xl font-bold">{stats.favoriteFramework}</div>
              <div className="text-sm text-gray-400">{t('multiFramework.stats.favorite', 'Favorite')}</div>
            </div>
          </div>
                ...
                {stats.recentProjects.map((p, i) => (
                  <div key={i} className="bg-gray-800 border border-gray-700 rounded-lg p-3 flex items-center justify-between">
                    <div>
                      ...
                      <span className="text-xs text-gray-400 ml-2">{p.framework}</span>
                    </div>
                    <span className="text-xs text-gray-500">{new Date(p.generatedAt).toLocaleDateString()}</span>
                  </div>
                ))}
              </div>
            </div>
