    ...
    <div className="space-y-6">
      ...
      {loading && (
        <div className="text-center py-8">
          ...
          <p className="text-gray-400 text-sm">{t('observability.loading', 'Loading...')}</p>
        </div>
      )}
        ...
        <div className="space-y-4">
          ...
          {traces.length === 0 ? (
            <div className="text-center py-12 bg-gray-800/50 rounded-lg">
              <p className="text-gray-400">{t('observability.noTraces', 'No traces found.')}</p>
            </div>
          ) : (
            <>
                  ...
                  <tbody>
                    {traces.map((tr) => (
                      ...
                      >
                        ...
                        <td className="py-2 px-2 text-gray-300">{tr.model || '-'}</td>
                        ...
                        <td className="py-2 px-2 text-gray-400 text-xs">{formatDate(tr.createdAt)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              {totalPages > 1 && (
                <div className="flex items-center justify-center gap-2 pt-2">
                  ...
                  <span className="text-sm text-gray-400">{tracePage} / {totalPages}</span>
                  <button
        ...
        <div className="space-y-4">
          ...
          <div className="bg-gray-800 rounded-xl p-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
              <div>
                <div className="text-xs text-gray-400">{t('observability.traceId', 'Trace ID')}</div>
                <div className="font-mono text-sm text-blue-400 break-all">{selectedTrace.traceId}</div>
              </div>
              <div>
                <div className="text-xs text-gray-400">{t('observability.tokens', 'Tokens')}</div>
                <div className="text-lg font-bold">{selectedTrace.totalTokens.toLocaleString()}</div>
              </div>
              <div>
                <div className="text-xs text-gray-400">{t('observability.cost', 'Cost')}</div>
                <div className="text-lg font-bold text-yellow-400">{formatCost(selectedTrace.totalCost)}</div>
              </div>
              <div>
                <div className="text-xs text-gray-400">{t('observability.latency', 'Latency')}</div>
            ...
            {selectedTrace.spans.length === 0 ? (
              <p className="text-gray-500 text-sm">{t('observability.noSpans', 'No spans recorded.')}</p>
            ) : (
              <div className="space-y-2">
                {selectedTrace.spans.map((span) => {
                  ...
                  return (
                    <div key={span.id} className="bg-gray-900 rounded-lg p-3">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-gradient-to-r from-yellow-900/50 to-orange-900/50 rounded-xl p-5">
              <div className="text-sm text-gray-300">{t('observability.totalCost', 'Total Cost')}</div>
              <div className="text-3xl font-bold text-yellow-400">{formatCost(costData.totalCost)}</div>
            </div>
            <div className="bg-gray-800 rounded-xl p-5">
              <div className="text-sm text-gray-300">{t('observability.totalTraces', 'Total Traces')}</div>
      ...
      {!loading && view === 'performance' && perfData && (
        <div className="space-y-4">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="bg-gray-800 rounded-xl p-5">
              <div className="text-sm text-gray-400">{t('observability.successRate', 'Success Rate')}</div>
            ...
            <div className="bg-gray-800 rounded-xl p-5">
              <div className="text-sm text-gray-400">{t('observability.avgLatency', 'Avg Latency')}</div>
              <div className="text-3xl font-bold">{formatLatency(perfData.avgLatencyMs)}</div>
            </div>
            <div className="bg-gray-800 rounded-xl p-5">
              <div className="text-sm text-gray-400">{t('observability.totalTraces', 'Total Traces')}</div>
              <div className="text-3xl font-bold">{perfData.totalTraces.toLocaleString()}</div>
            </div>
            <div className="bg-gray-800 rounded-xl p-5">
              <div className="text-sm text-gray-400">{t('observability.errors', 'Errors')}</div>
      ...
      {!loading && view === 'usage' && usageData && (
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className="bg-gray-800 rounded-xl p-5">
              <div className="text-sm text-gray-400">{t('observability.inputTokens', 'Input Tokens')}</div>
              <div className="text-2xl font-bold text-blue-400">{usageData.totalInputTokens.toLocaleString()}</div>
            </div>
            <div className="bg-gray-800 rounded-xl p-5">
              <div className="text-sm text-gray-400">{t('observability.outputTokens', 'Output Tokens')}</div>
              <div className="text-2xl font-bold text-purple-400">{usageData.totalOutputTokens.toLocaleString()}</div>
            </div>
            <div className="bg-gray-800 rounded-xl p-5">
              <div className="text-sm text-gray-400">{t('observability.totalTokens', 'Total Tokens')}</div>
