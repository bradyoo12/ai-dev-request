  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gray-900 rounded-lg p-6">
        ...
        <p className="text-gray-400 text-sm mb-4">{t('performanceProfile.subtitle')}</p>
      ...
      {profile && (
        <div className="bg-gray-900 rounded-lg p-4">
          <div className="flex items-center justify-between mb-3">
            <h4 className="text-sm font-medium text-gray-300">{t('performanceProfile.metrics')}</h4>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            <div className="bg-gray-800/50 rounded p-3">
              <p className="text-xs text-gray-500">{t('performanceProfile.suggestions')}</p>
              <p className="text-lg font-semibold text-white">{profile.suggestionCount}</p>
            </div>
            <div className="bg-gray-800/50 rounded p-3">
              <p className="text-xs text-gray-500">{t('performanceProfile.applied')}</p>
              <p className="text-lg font-semibold text-white">{profile.optimizationsApplied}</p>
            </div>
            <div className="bg-gray-800/50 rounded p-3">
              <p className="text-xs text-gray-500">{t('performanceProfile.status')}</p>
            ...
            <div className="bg-gray-800/50 rounded p-3">
              <p className="text-xs text-gray-500">{t('performanceProfile.pending')}</p>
      ...
      {profile && suggestions.length > 0 && (
        <div className="bg-gray-900 rounded-lg p-4">
          <div className="flex items-center justify-between mb-4">
            <h4 className="text-sm font-medium text-gray-300">{t('performanceProfile.optimizations')}</h4>
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      <span className={`px-2 py-0.5 rounded text-xs border ${categoryColors[suggestion.category] || 'bg-gray-600/20 text-gray-400 border-gray-700/50'}`}>
                        {suggestion.category}
                      </span>
                      <span className={`text-xs ${effortColors[suggestion.effort] || 'text-gray-400'}`}>
      ...
      {showHistory && history.length > 0 && (
        <div className="bg-gray-900 rounded-lg p-4">
          <h4 className="text-sm font-medium text-gray-300 mb-3">{t('performanceProfile.history')}</h4>
      ...
      {!profile && !profiling && !loading && (
        <div className="bg-gray-900 rounded-lg p-12 text-center">
          <p className="text-gray-400">{t('performanceProfile.empty')}</p>
        </div>
      )}
    </div>
  )
