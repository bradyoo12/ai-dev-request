...
export default function PlanningModePage() {
  ...
  const statusBadge = (status: string) => {
    const colors: Record<string, string> = {
      ...
      archived: 'bg-warm-700 text-warm-400',
    }
  ...
  const modeBadge = (mode: string) => {
    ...
    return colors[mode] || 'bg-warm-700 text-warm-400'
  }

  return (
    <div>
      <h3 className="text-xl font-bold mb-4">{t('planning.title', 'Discussion / Planning Mode')}</h3>
      <p className="text-warm-400 text-sm mb-6">{t('planning.subtitle', 'Brainstorm, plan architecture, and debug concepts with AI without generating code -- saving tokens significantly.')}</p>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button
                    ...
                    className="px-4 py-1.5 bg-warm-700 text-warm-300 rounded-lg text-sm hover:bg-warm-600 transition-colors"
                  >
              ...
              <div className="bg-warm-900 border border-warm-700 rounded-lg p-4 max-h-[500px] overflow-y-auto">
                {parseMessages(activeSession.messagesJson).length === 0 && (
                  <div className="text-center py-8 text-warm-500 text-sm">{t('planning.noMessages', 'Start the conversation by sending a message below.')}</div>
                )}
                    ...
                    <div
                      className={`max-w-[80%] rounded-lg p-3 text-sm whitespace-pre-wrap ${
                        msg.role === 'user'
                          ...
                          : 'bg-warm-800 text-warm-200 border border-warm-700'
                      }`}
                    >
                      {msg.content}
                    </div>
              ...
              {activeSession.status === 'active' && (
                <div className="flex gap-2">
                  <textarea
                    ...
                    rows={2}
                    className="flex-1 bg-warm-800 border border-warm-700 rounded-lg p-3 text-sm placeholder-warm-500 focus:border-blue-500 focus:outline-none resize-none"
                  />
                  <button
              ...
              {activeSession.status === 'completed' && parsePlanOutput(activeSession.planOutputJson) && (
                <div className="bg-warm-900 border border-green-800 rounded-lg p-4">
                  <h4 className="font-medium text-green-400 mb-3">{t('planning.planSummary', 'Plan Summary')}</h4>
                  <p className="text-sm text-warm-300 mb-3">{parsePlanOutput(activeSession.planOutputJson)!.summary}</p>
                  {parsePlanOutput(activeSession.planOutputJson)!.keyTopics.length > 0 && (
                    <div className="mb-3">
                      <span className="text-xs text-warm-400 font-medium">{t('planning.keyTopics', 'Key Topics')}:</span>
                      <div className="flex flex-wrap gap-1 mt-1">
                        {parsePlanOutput(activeSession.planOutputJson)!.keyTopics.map((topic, i) => (
                          <span key={i} className="text-xs bg-warm-800 border border-warm-700 rounded px-2 py-0.5">{topic}</span>
                        ))}
                      </div>
                    </div>
                  )}
                  {parsePlanOutput(activeSession.planOutputJson)!.nextSteps.length > 0 && (
                    <div>
                      <span className="text-xs text-warm-400 font-medium">{t('planning.nextSteps', 'Next Steps')}:</span>
                      <ul className="mt-1 space-y-1">
                        {parsePlanOutput(activeSession.planOutputJson)!.nextSteps.map((step, i) => (
                          <li key={i} className="text-xs text-warm-300 flex items-start gap-2">
              ...
              <div className="grid grid-cols-4 gap-3">
                <div className="bg-warm-800 border border-warm-700 rounded-lg p-3 text-center">
                  ...
                  <div className="text-xs text-warm-400">{t('planning.totalMessages', 'Messages')}</div>
                </div>
                <div className="bg-warm-800 border border-warm-700 rounded-lg p-3 text-center">
                  <div className="text-lg font-bold">{activeSession.userMessages}</div>
                  <div className="text-xs text-warm-400">{t('planning.userMessages', 'Your Messages')}</div>
                </div>
                <div className="bg-warm-800 border border-warm-700 rounded-lg p-3 text-center">
                  <div className="text-lg font-bold">{activeSession.tokensUsed.toLocaleString()}</div>
                  <div className="text-xs text-warm-400">{t('planning.tokensUsed', 'Tokens Used')}</div>
                </div>
                <div className="bg-warm-800 border border-warm-700 rounded-lg p-3 text-center">
                  <div className="text-lg font-bold text-green-400">{activeSession.estimatedSavings.toLocaleString()}</div>
                  <div className="text-xs text-warm-400">{t('planning.estSavings', 'Est. Savings')}</div>
                </div>
              </div>
            </div>
          )}
      ...
      {tab === 'sessions' && (
        <div className="space-y-3">
          {sessions.length === 0 && (
            <div className="text-center py-12 text-warm-500 text-sm">{t('planning.noSessions', 'No planning sessions yet. Start one from the Chat tab!')}</div>
          )}
          {viewSession && (
            <div className="bg-warm-900 border border-warm-700 rounded-lg p-4 mb-4">
              <div className="flex items-center justify-between mb-3">
                ...
                <button onClick={() => setViewSession(null)} className="text-sm text-warm-400 hover:text-white">Close</button>
              </div>
                ...
                {parseMessages(viewSession.messagesJson).map((msg, i) => (
                  <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                    <div className={`max-w-[80%] rounded-lg p-2 text-xs whitespace-pre-wrap ${
                      msg.role === 'user' ? 'bg-blue-600 text-white' : 'bg-warm-800 text-warm-200 border border-warm-700'
                    }`}>
                      {msg.content}
                    </div>
                  </div>
          ...
          {sessions.map((s) => (
            <div key={s.id} className="bg-warm-800 border border-warm-700 rounded-lg p-4 flex items-center justify-between">
              <div className="flex-1 cursor-pointer" onClick={() => setViewSession(s)}>
                <div className="font-medium text-sm">{s.sessionName}</div>
                <div className="text-xs text-warm-400 mt-1">{s.totalMessages} messages, {s.tokensUsed.toLocaleString()} tokens</div>
              </div>
              <div className="flex items-center gap-3">
                ...
                <span className="text-xs text-warm-500">{new Date(s.updatedAt).toLocaleDateString()}</span>
                <button
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            <div className="bg-warm-800 border border-warm-700 rounded-lg p-4 text-center">
              ...
              <div className="text-sm text-warm-400">{t('planning.stats.totalSessions', 'Total Sessions')}</div>
            </div>
            <div className="bg-warm-800 border border-warm-700 rounded-lg p-4 text-center">
              <div className="text-2xl font-bold text-blue-400">{stats.activeSessions}</div>
              <div className="text-sm text-warm-400">{t('planning.stats.active', 'Active')}</div>
            </div>
            <div className="bg-warm-800 border border-warm-700 rounded-lg p-4 text-center">
              <div className="text-2xl font-bold text-green-400">{stats.completedSessions}</div>
              <div className="text-sm text-warm-400">{t('planning.stats.completed', 'Completed')}</div>
            </div>
            <div className="bg-warm-800 border border-warm-700 rounded-lg p-4 text-center">
              <div className="text-2xl font-bold">{stats.totalMessages}</div>
              <div className="text-sm text-warm-400">{t('planning.stats.messages', 'Total Messages')}</div>
            </div>
            <div className="bg-warm-800 border border-warm-700 rounded-lg p-4 text-center">
              <div className="text-2xl font-bold">{stats.totalTokensUsed.toLocaleString()}</div>
              <div className="text-sm text-warm-400">{t('planning.stats.tokens', 'Tokens Used')}</div>
            </div>
            <div className="bg-warm-800 border border-warm-700 rounded-lg p-4 text-center">
              <div className="text-2xl font-bold text-green-400">{stats.estimatedSavings.toLocaleString()}</div>
              <div className="text-sm text-warm-400">{t('planning.stats.savings', 'Est. Token Savings')}</div>
            </div>
          </div>
          ...
          {stats.byMode.length > 0 && (
            <div>
              ...
              <div className="space-y-2">
                {stats.byMode.map((m) => (
                  <div key={m.mode} className="bg-warm-800 border border-warm-700 rounded-lg p-3 flex items-center justify-between">
                    <div className="flex items-center gap-3">
      ...
      {tab === 'stats' && !stats && (
        <div className="text-center py-12 text-warm-500 text-sm">{t('planning.stats.loading', 'Loading stats...')}</div>
      )}
    </div>
  )
}
