  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-warm-400 text-sm mt-1">{t('semanticSearch.subtitle', 'pgvector-powered semantic search for template matching and project recommendations')}</p>
      </div>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-warm-800 rounded-lg p-6 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm text-warm-400 mb-1">{t('semanticSearch.sourceType', 'Source Type')}</label>
                <select
                  value={newSourceType}
              ...
              <div>
                <label className="block text-sm text-warm-400 mb-1">{t('semanticSearch.sourceId', 'Source ID (optional)')}</label>
                <input
                  type="text"
                  value={newSourceId}
            ...
            <div>
              <label className="block text-sm text-warm-400 mb-1">{t('semanticSearch.itemTitle', 'Title')}</label>
              <input
                type="text"
                value={newTitle}
            ...
            <div>
              <label className="block text-sm text-warm-400 mb-1">{t('semanticSearch.content', 'Content')}</label>
              <textarea
                value={newContent}
                onChange={e => setNewContent(e.target.value)}
                rows={4}
                className="w-full bg-warm-700 border border-warm-600 text-white rounded-md px-3 py-2 text-sm resize-y"
          ...
          <div className="flex items-center gap-3">
            <label className="text-sm text-warm-400">{t('semanticSearch.filterBy', 'Filter by:')}</label>
            <div className="flex gap-2">
              {SOURCE_TYPES.map(st => (
                <button
                  key={st}
          ...
          {loading ? (
            ...
            <div className="space-y-2">
              {indexItems.map(item => (
                <div key={item.id} className="bg-warm-800 rounded-lg p-4 flex items-center justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      <span className="text-white font-medium text-sm">{item.title}</span>
                      <span className={`px-2 py-0.5 rounded-full text-xs ${SOURCE_TYPE_COLORS[item.sourceType] || 'bg-warm-700 text-warm-300'}`}>
                        {item.sourceType}
                      </span>
                    </div>
      ...
      {tab === 'search' && (
        <div className="space-y-4">
          <div className="bg-warm-800 rounded-lg p-6 space-y-4">
            ...
            <div>
              <label className="block text-sm text-warm-400 mb-1">{t('semanticSearch.queryText', 'Query Text')}</label>
              <textarea
                value={queryText}
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm text-warm-400 mb-1">{t('semanticSearch.sourceTypeFilter', 'Source Type Filter')}</label>
                <select
              ...
              <div>
                <label className="block text-sm text-warm-400 mb-1">
          ...
          {searchResults.length > 0 && (
            <div className="space-y-2">
              ...
              {searchResults.map((result, idx) => (
                <div key={result.id} className="bg-warm-800 rounded-lg p-4 space-y-2">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <span className="text-warm-500 text-xs font-mono">#{idx + 1}</span>
                      <span className="text-white font-medium text-sm">{result.title}</span>
                      <span className={`px-2 py-0.5 rounded-full text-xs ${SOURCE_TYPE_COLORS[result.sourceType] || 'bg-warm-700 text-warm-300'}`}>
                        {result.sourceType}
                      </span>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="text-xs text-warm-400">{getSimilarityLabel(result.similarity)}</span>
                  ...
                  <p className="text-warm-400 text-xs line-clamp-2">{result.content}</p>
            ...
            <>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="bg-warm-800 rounded-lg p-4">
                  <div className="text-warm-400 text-xs uppercase tracking-wider">{t('semanticSearch.totalIndexed', 'Total Indexed')}</div>
                  <div className="text-2xl font-bold text-white mt-1">{stats.totalIndexed}</div>
                </div>
                <div className="bg-warm-800 rounded-lg p-4">
                  <div className="text-warm-400 text-xs uppercase tracking-wider">{t('semanticSearch.dimensions', 'Dimensions')}</div>
                  <div className="text-2xl font-bold text-white mt-1">{stats.dimensions}</div>
                </div>
                <div className="bg-warm-800 rounded-lg p-4">
                  <div className="text-warm-400 text-xs uppercase tracking-wider">{t('semanticSearch.contentSize', 'Content Size')}</div>
                  <div className="text-2xl font-bold text-white mt-1">
                    {stats.totalContentSize > 1024 ? `${(stats.totalContentSize / 1024).toFixed(1)} KB` : `${stats.totalContentSize} B`}
                  </div>
                </div>
                <div className="bg-warm-800 rounded-lg p-4">
                  <div className="text-warm-400 text-xs uppercase tracking-wider">{t('semanticSearch.lastIndexed', 'Last Indexed')}</div>
              ...
              <div className="bg-warm-800 rounded-lg p-6">
                ...
                {stats.bySourceType.length === 0 ? (
                  <p className="text-warm-500 text-sm">{t('semanticSearch.noData', 'No data yet')}</p>
                ) : (
                  <div className="space-y-3">
                    {stats.bySourceType.map(entry => (
                      <div key={entry.sourceType} className="flex items-center gap-3">
                        <span className={`px-2 py-0.5 rounded-full text-xs min-w-[80px] text-center ${SOURCE_TYPE_COLORS[entry.sourceType] || 'bg-warm-700 text-warm-300'}`}>
                          {entry.sourceType}
                        </span>
