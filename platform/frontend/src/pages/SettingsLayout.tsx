...
export default function SettingsLayout() {
  ...
  const pathTab = location.pathname === '/settings/specifications' ? 'specifications' as SettingsTab
    ...
    : location.pathname === '/settings/streaming-preview' ? 'streaming-preview' as SettingsTab : null
  ...
  return (
    <section>
      ...
      <div className="flex gap-1 mb-6 bg-gray-800 rounded-lg p-1 overflow-x-auto whitespace-nowrap">
        ...
        <button
          onClick={() => setSettingsTab('streaming-preview')}
          className={`flex-1 py-2 px-4 rounded-md text-sm font-medium transition-colors ${
            settingsTab === 'streaming-preview' ? 'bg-gray-700 text-white' : 'text-gray-400 hover:text-white'
          }`}
        >
          <span className="flex items-center gap-1 justify-center">
            <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polygon points="6 3 20 12 6 21 6 3"/><line x1="20" y1="3" x2="20" y2="21"/></svg>
            {t('settings.tabs.streamingPreview', 'Streaming Preview')}
          </span>
        </button>
      </div>
      ...
      {settingsTab === 'streaming-preview' && <StreamingPreviewPage />}
    </section>
  )
}
