...
export default function SettingsLayout() {
  ...
  const pathTab = location.pathname === '/settings/specifications' ? 'specifications' as SettingsTab
    ...
    : location.pathname === '/settings/test-healing' ? 'test-healing' as SettingsTab
  ...
  return (
    <section>
      ...
      <div className="flex gap-1 mb-6 bg-warm-800 rounded-lg p-1 overflow-x-auto whitespace-nowrap" role="tablist" aria-label={t('settings.title')}>
        ...
        <button
          onClick={() => setSettingsTab('test-healing')}
          className={`flex-1 py-2 px-4 rounded-md text-sm font-medium transition-colors ${
            settingsTab === 'test-healing' ? 'bg-warm-700 text-white' : 'text-warm-400 hover:text-white'
          }`}
        >
          <span className="flex items-center gap-1 justify-center">
            <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M19 14c1.49-1.46 3-3.21 3-5.5A5.5 5.5 0 0 0 16.5 3c-1.76 0-3 .5-4.5 2-1.5-1.5-2.74-2-4.5-2A5.5 5.5 0 0 0 2 8.5c0 2.3 1.5 4.05 3 5.5l7 7Z"/><path d="m9 12 2 2 4-4"/></svg>
            {t('settings.tabs.testHealing', 'Self-Healing')}
          </span>
        </button>
      </div>
      ...
      {settingsTab === 'test-healing' && <TestHealingPage />}
    </section>
  )
}
