  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-400 text-sm mt-1">{t('visualOverlay.subtitle', 'Point-and-click editing for generated project UI components')}</p>
      </div>
        ...
        <div className="space-y-4">
          ...
          {!currentSession && (
            <div className="bg-gray-800 rounded-lg p-6 space-y-4">
              ...
              <div>
                <label className="block text-sm text-gray-400 mb-1">{t('visualOverlay.projectName', 'Project Name')}</label>
                <input
                  type="text"
                  value={projectName}
              ...
              <div>
                <label className="block text-sm text-gray-400 mb-1">{t('visualOverlay.previewUrl', 'Preview URL')}</label>
                <input
                  type="text"
                  value={previewUrl}
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <span className={`text-xs px-2 py-0.5 rounded ${STATUS_COLORS[currentSession.status] || 'bg-gray-700 text-gray-300'}`}>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                ...
                <div className="bg-gray-800 rounded-lg p-4">
                  <h5 className="text-sm font-medium text-gray-300 mb-3">{t('visualOverlay.componentTree', 'Component Tree')}</h5>
                ...
                <div className="bg-gray-800 rounded-lg p-4">
                  <h5 className="text-sm font-medium text-gray-300 mb-3">{t('visualOverlay.preview', 'Preview')}</h5>
                  <div className="bg-gray-900 rounded-lg border border-gray-700 h-64 flex items-center justify-center">
                    {currentSession.previewUrl ? (
                      <div className="text-center">
                        <p className="text-gray-400 text-sm">{t('visualOverlay.previewLabel', 'Preview')}:</p>
                      ...
                      <div className="text-center">
                        <p className="text-gray-500 text-sm">{t('visualOverlay.noPreview', 'No preview URL provided')}</p>
                        {selectedElement && (
                          <div className="mt-3 px-3 py-1.5 bg-blue-900 rounded text-blue-300 text-xs">
                            {t('visualOverlay.selected', 'Selected')}: {selectedElement}
                          </div>
                ...
                <div className="bg-gray-800 rounded-lg p-4">
                  <h5 className="text-sm font-medium text-gray-300 mb-3">{t('visualOverlay.propertyEditor', 'Property Editor')}</h5>
                  {selectedElement ? (
                    <div className="space-y-3">
                      <div>
                        <label className="block text-xs text-gray-500 mb-1">{t('visualOverlay.element', 'Element')}</label>
                        <p className="text-sm text-blue-400 font-mono">{selectedElement}</p>
                      </div>
                      <div>
                        <label className="block text-xs text-gray-500 mb-1">{t('visualOverlay.property', 'Property')}</label>
                        <select
                          value={editProperty}
                          onChange={e => setEditProperty(e.target.value)}
                          className="w-full bg-gray-700 border border-gray-600 text-white rounded-md px-3 py-1.5 text-sm"
                        >
                      ...
                      {editProperty && (
                        <div>
                          <label className="block text-xs text-gray-500 mb-1">{t('visualOverlay.value', 'Value')}</label>
                    ...
                    <p className="text-gray-500 text-sm">{t('visualOverlay.selectElementHint', 'Select an element from the component tree to edit its properties')}</p>
                  )}
                </div>
              </div>
              ...
              <div className="bg-gray-800 rounded-lg p-4">
                <div className="flex items-center justify-between mb-3">
                  <h5 className="text-sm font-medium text-gray-300">{t('visualOverlay.modificationHistory', 'Modification History')}</h5>
                  <div className="flex items-center gap-2">
                    <span className="text-xs text-gray-500">{modifications.length} {t('visualOverlay.edits', 'edits')}</span>
                ...
                {modifications.length === 0 ? (
                  <p className="text-gray-500 text-sm">{t('visualOverlay.noModifications', 'No modifications yet. Select an element and edit its properties.')}</p>
                ) : (
                  <div className="space-y-2 max-h-40 overflow-auto">
                    {[...modifications].reverse().map((mod, idx) => (
                      <div key={idx} className="flex items-center justify-between text-sm bg-gray-900 rounded px-3 py-2">
                        <div className="flex items-center gap-2">
      ...
      {tab === 'sessions' && (
        <div className="space-y-4">
          {loading ? (
            <p className="text-gray-400 text-sm">{t('visualOverlay.loading', 'Loading...')}</p>
          ) : sessions.length === 0 ? (
            <div className="bg-gray-800 rounded-lg p-8 text-center">
              <p className="text-gray-400">{t('visualOverlay.noSessions', 'No editing sessions yet. Start your first visual overlay session!')}</p>
            </div>
          ) : (
            <div className="space-y-3">
              {sessions.map(s => (
                  ...
                  <div className="flex justify-between items-start gap-4">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        <span className={`text-xs px-2 py-0.5 rounded ${STATUS_COLORS[s.status] || 'bg-gray-700 text-gray-300'}`}>
                          {s.status}
                        </span>
                        <span className="text-xs text-gray-500">{s.totalEdits} {t('visualOverlay.edits', 'edits')}</span>
                      </div>
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-3 gap-4">
            <div className="bg-gray-800 rounded-lg p-4 text-center">
              ...
              <p className="text-gray-400 text-sm">{t('visualOverlay.stats.totalSessions', 'Total Sessions')}</p>
            </div>
            <div className="bg-gray-800 rounded-lg p-4 text-center">
              <p className="text-2xl font-bold text-green-400">{stats.totalEdits}</p>
              <p className="text-gray-400 text-sm">{t('visualOverlay.stats.totalEdits', 'Total Edits')}</p>
            </div>
            <div className="bg-gray-800 rounded-lg p-4 text-center">
              <p className="text-2xl font-bold text-blue-400">{stats.avgEditsPerSession}</p>
              <p className="text-gray-400 text-sm">{t('visualOverlay.stats.avgEdits', 'Avg Edits/Session')}</p>
            </div>
          </div>
          <div className="grid grid-cols-3 gap-4">
            <div className="bg-gray-800 rounded-lg p-4 text-center">
              <p className="text-2xl font-bold text-purple-400">{stats.mostEditedProperty}</p>
              <p className="text-gray-400 text-sm">{t('visualOverlay.stats.mostEdited', 'Most Edited Property')}</p>
            </div>
            <div className="bg-gray-800 rounded-lg p-4 text-center">
              <p className="text-2xl font-bold text-yellow-400">{stats.activeSessions}</p>
              <p className="text-gray-400 text-sm">{t('visualOverlay.stats.activeSessions', 'Active Sessions')}</p>
            </div>
            <div className="bg-gray-800 rounded-lg p-4 text-center">
              <p className="text-2xl font-bold text-orange-400">
                {stats.lastSession ? new Date(stats.lastSession).toLocaleDateString() : 'N/A'}
              </p>
              <p className="text-gray-400 text-sm">{t('visualOverlay.stats.lastSession', 'Last Session')}</p>
            </div>
          </div>
        </div>
      )}
